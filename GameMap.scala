...
class GameMap(val player: Player) {
  ...
  val sheriffOffice = new Room("abandonned sheriff's office", false, town, player)
  ...
  val angryWhiskey = new Item("bottle of angerin' whiskey (damage +2)", worth = 80, canSell = true)
  angryWhiskey.buffType = "damage"
  angryWhiskey.buffAmt = 2
  angryWhiskey.drink = true
  bartender.addItem(angryWhiskey)
  ...
  banditTreasureRoom.addItem(new Item("almost empty treasure chest", worth = 50, canSell = true))
  ...
  attackPot.buffAmt = 6
  attackPot.drink = true
  wizard.addItem(attackPot)

  val healthkPot = new Item("health pot, +25 hp", worth = 200,consumable = true, canSell = true)
  healthkPot.buffType = "health"
  healthkPot.buffAmt = 25
  healthkPot.drink = true
  ...
  thor.addDialogue("hey there, adventurer. I'm selling the most powerful potion known to man, which will turn you into a god. it'll cost ya, though (type /shop to shop)")
  ...
  thorRoom.buildDoor(outsideThor)
  thorRoom.addNPC(thor)

  var gamblersDen = new Area("gambler's den")
  var gamblerOutside = new Room("outside gambler's den", false, gamblersDen, player)
  var gamblerInside  = new Room("inside gambler's den", false, gamblersDen, player)
  ...
  def playPoker(): Unit = {
    ...
    while (!quit) {
      if(!skipRound) {
        ...
        if (oddsOfWinning < 15) {
          println(s"you draw 3 terrible cards. You estimate your odds of winning to be $oddsOfWinning%")
        }
        else if (oddsOfWinning < 35) {
          println(s"you draw 2 terrible cards and 1 good card. You estimate your odds of winning to be $oddsOfWinning%")
        }
        else if (oddsOfWinning < 55) {
          println(s"you draw 3 fairly okay cards. You estimate your odds of winning to be $oddsOfWinning%")
        }
        else if (oddsOfWinning < 75) {
          println(s"you draw 3 pretty good cards. You estimate your odds of winning to be $oddsOfWinning%")
        }
        else if (oddsOfWinning < 95) {
          println(s"you draw 3 good cards. You estimate your odds of winning to be $oddsOfWinning%")
        }
        else {
          println(s"you draw 3 amazing cards. You estimate your odds of winning to be $oddsOfWinning%")
        }
  ...
  def enterRoom(room: String): Boolean = {
    ...
    if (!found) {
      if (!currentArea.rooms.exists(_.name == room)) println(s"room ${room} not found")
      else println(s"You cannot enter the room $room from ${currentArea.currentRoom.name}")
    }
    found
  }

  def finalFight(): Unit =  {
    println("bartender: muahahahaha, you fell into my eeeevil trap. This shovel makes me very strong.")
    println("I hope you came prepared...")
    readLine()
    println("you fool...")
    val ans = readLine("although, if you came to play poker, go right ahead (y/n)\n>>> ")
    if (ans.nonEmpty  && ans.head.toLower == 'y') {
      playPoker()
      return ()
    }
