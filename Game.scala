...
class Game {
  ...
  var useLevenstein = true
  val gameMap = new GameMap(player)
  if (player.name.toLowerCase == "chuck norris") {
    gameOver()
  }
  else {
    initCharacter()
  ...
  def initGame(): Player = {
    ...
    if (name.toLowerCase == "chuck norris") {
      return new Player(name, new Area("Home"))
    }
  ...
  def playGame() = {
    ...
    println("You've just learnt the /help command! great!\nAreas are subdivided into rooms. move between rooms using \"/gotoroom <roomname>\". You can find information about the room using \"/exploreroom\"")
  ...
  private def excecuteCommand() = {
    ...
    var commands = Vector("/explore", "/explorearea", "/exploreroom", "/movetoarea", "/gotoarea", "/gotoroom", "/movetoroom", "/pickup", "/wander", "/help", "/shop", "/inventory", "/drink", "/eat", "/stats")
    if (gameMap.canEnchant) commands :+= "/turnintogold"

    if (!commands.contains(command)) {
      command = returnCorrect(command, commands, useLevenstein)
    }
    ...
    command match {
      ...
      case "/movetoarea"   => {
        if (player.items.exists(_.name == "map with path to town")) {
          ...
          if (!filteredAreas.map(_.name.toLowerCase).contains(rest)) {
            rest = returnCorrect(rest, filteredAreas.map(_.name), useLevenstein)
          }
      ...
      case "/movetoroom"   => {
        ...
        if (!rooms.map(_.name.toLowerCase).contains(rest)) {
          rest = returnCorrect(rest, rooms.map(_.name), useLevenstein)
        }
      ...
      case "/pickup"       => {
        if (!gameMap.currentArea.currentRoom.itemsInRoom.map(_.name).contains(rest)) {
          rest = returnCorrect(rest, gameMap.currentArea.currentRoom.itemsInRoom.map(_.name), useLevenstein)
        }
      ...
      case "/turnintogold"      => {
        if (gameMap.canEnchant) {
          if (!player.items.map(_.name).contains(rest)) rest = returnCorrect(rest, player.items.map(_.name), useLevenstein)
          gameMap.enchant(rest)
        } else println("command not recognised.")
      }
      case "/drink"        => {
        if (!player.items.exists(_.name == rest)) rest = returnCorrect(rest, player.items.filter(i => i.consumable && i.drink).map(_.name), useLevenstein)
        if (player.items.exists(i => i.name == rest && i.drink && i.consumable)) {
      ...
      case "/eat"        => {
        ...
        } else println(s"food ${rest} not recognised")
      }
      case "/stats" => stats()
  ...
  def stats() = {
    println(s"damage: ${player.perks.damage._1}. ${player.perks.damage._2}")
    println(s"resistance: ${player.perks.resistance._1}. ${player.perks.resistance._2}")
    println(s"health: ${player.perks.health._1}. ${player.perks.health._2}")
    println(s"apetite: ${player.perks.apetite._1}. ${player.perks.apetite._2}")
    println(s"thirst: ${player.perks.mouthAriditySpotsLeft._1}. ${player.perks.mouthAriditySpotsLeft._2}")
    println(s"lives left: ${player.perks.livesLeft}. ${player.perks.lifeMessages(player.perks.livesLeft)}")
  }
  ...
  def help() = {
    ...
    if (gameMap.canEnchant) println("/turnintogold <item>: turn specified item into gold")
  }

  def settings() = {
    println("settings")
    var word = readLine("Turn color bling mode on? (y/n)\n(settings) >>> ")
    if (word.nonEmpty && word.head.toLower == 'y') {
      println("color bling mode is now on")
      colorBlindMode = true
    }
    else if (word.nonEmpty && word.head.toLower == 'n') {
      println("color bling mode is now off")
      colorBlindMode = false
    }
    else println("input not recognised")
    word = readLine("Use levenstein distance for word correction? (y/n)\n(settings) >>> ")
    if (word.nonEmpty && word.head.toLower == 'y') {
      println("Only using string comparison now")
      useLevenstein = true
    }
    else if (word.nonEmpty && word.head.toLower == 'n') {
      println("Using levenstein distanse for word correction")
      useLevenstein = false
    }
    else println("input not recognised")
  }
  ...
  def gameOver() = {
    ...
    if (player.name.toLowerCase == "chuck norris") println("All the enemies shiver before you and give up (i mean, you are Chuck Norris after all). You beat the game")
    else {
